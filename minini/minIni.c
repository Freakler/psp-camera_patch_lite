...
int _puts(const char *s, SceUID stream) {
	return sceIoWrite(stream, s, strlen(s));
}

char *_gets(char *s, int size, SceUID stream) {
    int ret = sceIoRead(stream, s, size - 1);
    if(ret <= 0)
        return NULL;
    int i = 0;
    while(i < ret) {
    ...
    s[i] = '\0';
    if(ret - i)
        sceIoLseek32(stream, -(ret - i), PSP_SEEK_CUR);
    return s;
}
...
static TCHAR *save_strncpy(TCHAR *dest, const TCHAR *source, int maxlen, enum quote_option option)
{
  int d, s;
